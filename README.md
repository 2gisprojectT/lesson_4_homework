...
В рамках анализа мы должны получить карту функциональности и карту архитектуры, выявить риски на основе анализа истории ошибок, выявить приоритет функционала, то есть что нужно протестировать наиболее тщательно, определить критичность потенциальных ошибок (это может влиять на приоритизацию тестов).
На выходе нашего анализа будет: 
-карта функционала и карта архитектуры; 
-будут найдены взаимосвязи блоков тестирования, то есть что с чем важно тестировать;
-будет выявлена приоритизация тестирования, то есть что важнее всего протестировать в первую очередь;
-будут определены типы тестирования;
-будут приоритизированы условия и окружения;
-будут определены слабые места продукта, то есть что чаще всего ломается;
-будет определено что критичнее всего для пользователя, то есть что наиболее важно для пользователя в нашем продукте.
