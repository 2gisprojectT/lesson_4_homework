...
### Попытаться использовать изученные техники тест-дизайна и дополнить тесты из домашки 2
 - Выполнялость ранее в домашней работе 2
...
1. почему защита 1 уровня "что-нибудь потестировать" не работает?
 - у тестировщика (сотрудника) не достаточно знаний, часть ошибок не находится
 - отсутствие целостности
 - предвзятость в выборе
 - неизбежность пропусков

2. почему не помогает просто написать больше тестов?
 - возможно ошибочное представление об тестировании в целом
 - отсуствие документации
 - большие трудозатраты
 - проблема актуализации в процессе разработки
 - отсутствие целостности

3. зачем нужно анализировать?
 - выяснение и дальнейшая минимизация рисков, проблем с архитектурой, приотезация тестов по критичности
 - появляться адаптивность тестов 
 - пропускается меньше ошибок
...
6. зачем расставлять приоритеты? от чего зависит приоритет функциональности? на основании чего можно определить приоритет функциональности?
 - для определения порядка тестирования и уменьшения рисков при релизе
 - приоритет зависит от важности фукционала, с точки зрения его неоходимости для пользователя
 - как часто ошибки в функционале и без чего пользователь в случае "чего" сможет жить, а без чего нет

7. какие бывают внещние факторы, которые могут повлиять на тестирование? зачем учитывать внешние факторы?
 - длительность итераций, регулярность публичных релизов, ограничение по времени на тестирование, допустимость и недопустимость ошибок, возможности автоматизации 
 - внешние факторы влияют на процесс тестирования, их необходимо учитывать для того, чтобы он (процесс тестирования) соотвествовал плану
...
8. охаректеризуйте разные комплекты тестов (зачем? как выбрать тесты в комплект? как часто прогонять? в чем задача каждого набора? нужно проводить для одной сборки или можно на нескольких? и тд): BVT, UAT, FTP? какие интересные особенности для каждого комплекта вы запомнили?
 ...
 - FTP (Full Test Path) - позволяет собрать полную информацию о тестируемом продукте и определить контроль качества продукта. Сюда входят все возможные и невозможные тесты
 - (UAT - User Acceptance Testing) – тестирование, которое проводится конечными пользователями системы с целью принятия решения о внедрении. Позволяет определить готов ли продукт к релизу

9. что делать, если пропустили баг в релиз
 - понять почему пропустили ошибку
 - определить насколько критична ошибка
